#!/usr/bin/env python3
"""
Generic Requests + BeautifulSoup scraper for single/detail pages.
...
def crawl_detail_pages(session, start_url, detail_cfg, timeout, sleep_seconds):
    """
    Supports pagination inside the detail page itself if next_page is defined.
    """
    results = []
    visited = set()
    current_url = start_url
    max_pages = int(detail_cfg.get("max_pages", 1))
    ...
    for _ in range(max_pages):
        ...
        results.append((current_url, soup))

        next_page_spec = detail_cfg.get("next_page")
...
def scrape_detail_url(session, url, cfg):
    ...
    pages = crawl_detail_pages(session, url, detail_cfg, timeout, sleep_seconds)

    item = {
        "source_id": cfg["source_id"],
        "source_type": cfg.get("source_type", "detail_page"),
        "url": url,
    }

    # page-level fields: keep first non-null occurrence
...
def main():
    ...
    for url in cfg.get("start_urls", []):
        try:
            item = scrape_detail_url(session, url, cfg)
